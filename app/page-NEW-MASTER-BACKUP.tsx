...
// Back Work Order System labels + office pipeline counts
        ...
        <main className="mt-5">
          ...
          {(activeTab === "office" || activeTab === "invoices") && (
            <section className="space-y-4">
              ...
              <InvoicesPanel invoices={filteredInvoices} jobs={state.jobs} weekJobs={weekJobs} assignments={state.assignments} makeReady={state.makeReady} onAdd={() => { setEditingInvoice(null); setShowInvoiceForm(true); }} onCreateFromWeek={createInvoiceFromWeekJobs} onEdit={(invoice) => { setEditingInvoice(invoice); setShowInvoiceForm(true); }} onDelete={(id) => setConfirmDelete({ type: "invoice", id })} onUpdate={upsertInvoice} />
            </section>
          )}
...
function InvoicesPanel({ invoices, jobs, weekJobs, assignments, makeReady, onAdd, onCreateFromWeek, onEdit, onDelete, onUpdate }: { invoices: Invoice[]; jobs: JobEntry[]; weekJobs: JobEntry[]; assignments: WorkAssignment[]; makeReady: MakeReadyItem[]; onAdd: () => void; onCreateFromWeek: () => void; onEdit: (invoice: Invoice) => void; onDelete: (id: string) => void; onUpdate: (invoice: Invoice) => void }) {
  ...
  const weekJobIdsAlreadyInvoiced = new Set(invoices.flatMap((invoice) => invoice.sourceJobIds || []));
  const readyJobs = weekJobs.filter((job) => !weekJobIdsAlreadyInvoiced.has(job.id));
  const readyAssignments = assignments.filter((assignment) => assignment.status === "ready-to-invoice" || assignment.status === "approved" || assignment.status === "completed");
  const readyMakeReady = makeReady.filter((item) => item.status === "ready");
  const backOfficeReadyCount = readyJobs.length + readyAssignments.length + readyMakeReady.length;
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        <MiniMetric label="Ready" value={backOfficeReadyCount} />
      ...
      {backOfficeReadyCount > 0 && <div className="rounded-2xl border border-green-400/20 bg-green-500/10 p-3 text-sm font-semibold text-green-200">Back Office has {backOfficeReadyCount} item(s) ready: {readyJobs.length} job(s), {readyAssignments.length} assignment(s), and {readyMakeReady.length} make ready unit(s). Invoice counts stay at zero until an invoice is actually created/saved.</div>}
      {readyJobs.length > 0 && <button type="button" onClick={onCreateFromWeek} className="goldButton w-full"><Sparkles size={18} /> Create Invoice From {readyJobs.length} Ready Job(s)</button>}
      ...
      <div className="space-y-3">
        ...
        {visibleInvoices.length === 0 && <div className="blackCard p-6"><EmptyText text={invoiceFilter === "paid" ? "No paid invoices yet." : invoiceFilter === "outstanding" ? "No saved outstanding invoices yet." : backOfficeReadyCount > 0 ? "No saved invoices yet, but your Field work is showing above as ready for Back Office billing." : "No invoices yet. Create one manually or from this week’s jobs."} /></div>}
      </div>
    </section>
  );
}
...
function JobModal({ employees, properties, jobTypeOptions, getAddressForProperty, onAddProperty, onClose, onSave }: { employees: Employee[]; properties: string[]; jobTypeOptions: string[]; getAddressForProperty: (property: string) => string; onAddProperty: (property: string) => void; onClose: () => void; onSave: (job: JobEntry, assignment?: WorkAssignment) => void }) {
  ...
  const [selectedJobPreset, setSelectedJobPreset] = useState("");
  ...
  function applyFieldPreset(label: string) {
    setSelectedJobPreset(label);
    if (!label) {
      setSelectedTypes([]);
      setCustomWork("");
      return;
    ...
    setCustomWork(assignmentScopeFor(label, assignmentLanguage));
  }
