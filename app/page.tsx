...
// Payroll + Weekly Jobs + Make Ready Board + Invoice Builder
...
export default function PayrollProEliteOperationsX() {
  ...
  function markEmployeeWeekPaid(employeeId: string) {
    const ok = window.confirm("Mark this employee paid for the selected week?");
    if (!ok) return;
    setState((prev) => ({
      ...prev,
      jobs: prev.jobs.map((job) => {
    ...
    <div className={appShellClass}>
      <div className="mx-auto max-w-[540px] px-4 pb-28 pt-3">
        ...
        <main className="mt-5">
            ...
            <section className="space-y-4">
              ...
              <div className="grid gap-3">
                {filteredEmployees.map((employee) => {
                  ...
                  return <EmployeeCard key={employee.id} employee={employee} totals={row} expanded={expandedEmployeeId === employee.id} onToggle={() => setExpandedEmployeeId(expandedEmployeeId === employee.id ? null : employee.id)} onDelete={() => setConfirmDelete({ type: "employee", id: employee.id })} onSave={upsertEmployee} onMarkWeekPaid={markEmployeeWeekPaid} weekStart={week.start} />;
                })}
              </div>
            </section>
          )}
          ...
          {activeTab === "invoices" && (
            <InvoicesPanel invoices={filteredInvoices} weekJobs={weekJobs} onAdd={() => { setEditingInvoice(null); setShowInvoiceForm(true); }} onCreateFromWeek={createInvoiceFromWeekJobs} onEdit={(invoice) => { setEditingInvoice(invoice); setShowInvoiceForm(true); }} onDelete={(id) => setConfirmDelete({ type: "invoice", id })} onUpdate={upsertInvoice} />
          )}
...
function StatCard({ label, value, icon, description, variant = "earned" }: { label: string; value: string; icon: React.ReactNode; description?: string; variant?: "earned" | "paid" | "borrowed" | "owed" }) {
  ...
  const labelColor = variant === "earned" ? "text-green-400" : variant === "paid" ? "text-blue-400" : variant === "borrowed" ? "text-purple-400" : "text-orange-400";
...
function MakeReadyCard({ item, employee, employees, onEdit, onDelete, onUpdate }: { item: MakeReadyItem; employee?: Employee; employees: Employee[]; onEdit: () => void; onDelete: () => void; onUpdate: (item: MakeReadyItem) => void }) {
  ...
  return <div className="blackCard p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-lg font-black">{makeReadyTitle(item)}</p><p className="text-xs text-zinc-500">Assigned: {employee?.name || "Not assigned"} • Deadline: {item.deadline || "—"}</p></div><span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase ${statusClasses}`}>{item.priority === "urgent" ? "Urgent" : item.status.replace("-", " ")}</span></div><div className="mt-3 h-3 overflow-hidden rounded-full border border-white/10 bg-black/40"><div className="h-full rounded-full bg-green-500" style={{ width: `${percent}%` }} /></div><p className="mt-1 text-xs font-bold text-zinc-500">{percent}% complete • Move-out {item.moveOutDate || "—"} • Move-in {item.moveInDate || "—"}</p><div className="mt-3 grid gap-2">{item.tasks.map((task) => <button type="button" key={task.id} onClick={() => onUpdate({ ...item, tasks: item.tasks.map((row) => row.id === task.id ? { ...row, done: !row.done } : row) })} className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-left text-sm font-bold ${task.done ? "border-green-400/25 bg-green-500/10 text-green-300" : "border-zinc-800 bg-black/30 text-zinc-400"}`}><span>{task.done ? "✅" : "⬜"}</span>{task.label}</button>)}</div><div className="mt-3 grid grid-cols-2 gap-2"><Field label="Status"><select className="inputElite" value={item.status} onChange={(e) => onUpdate({ ...item, status: e.target.value as MakeReadyItem["status"] })}><option value="scheduled">Scheduled</option><option value="in-progress">In Progress</option><option value="waiting">Waiting</option><option value="ready">Ready</option></select></Field><Field label="Assigned"><select className="inputElite" value={item.assignedEmployeeId} onChange={(e) => onUpdate({ ...item, assignedEmployeeId: e.target.value })}><option value="">Not assigned</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select></Field></div>{item.notes && <p className="mt-3 rounded-2xl border border-white/10 bg-black/25 p-3 text-sm text-zinc-400">{item.notes}</p>}<div className="mt-3 flex gap-2"><button className="darkButton flex-1" onClick={onEdit}><Pencil size={16} /> Edit Unit</button><button className="iconDanger" onClick={onDelete}><Trash2 size={18} /></button></div></div>;
}

function InvoicesPanel({ invoices, weekJobs, onAdd, onCreateFromWeek, onEdit, onDelete, onUpdate }: { invoices: Invoice[]; weekJobs: JobEntry[]; onAdd: () => void; onCreateFromWeek: () => void; onEdit: (invoice: Invoice) => void; onDelete: (id: string) => void; onUpdate: (invoice: Invoice) => void }) {
  return <section className="space-y-4"><SectionTop title="Invoices" subtitle="Create invoices inside the app and track what is paid or still open."><div className="flex flex-col gap-2"><button onClick={onAdd} className="goldButton"><Plus size={18} /> New Invoice</button><button onClick={onCreateFromWeek} className="darkButton"><Sparkles size={16} /> Invoice This Week</button></div></SectionTop>{weekJobs.length > 0 && <div className="rounded-2xl border border-green-400/20 bg-green-500/10 p-3 text-sm font-semibold text-green-200">Selected week has {weekJobs.length} jobs ready to turn into an invoice.</div>}<div className="space-y-3">{invoices.map((invoice) => <InvoiceCard key={invoice.id} invoice={invoice} onEdit={() => onEdit(invoice)} onDelete={() => onDelete(invoice.id)} onUpdate={onUpdate} />)}{invoices.length === 0 && <div className="blackCard p-6"><EmptyText text="No invoices yet. Create one manually or from this week’s jobs." /></div>}</div></section>;
}

function InvoiceCard({ invoice, onEdit, onDelete, onUpdate }: { invoice: Invoice; onEdit: () => void; onDelete: () => void; onUpdate: (invoice: Invoice) => void }) {
  const total = invoiceTotal(invoice);
  const open = Math.max(total - safeNumber(invoice.paidAmount), 0);
  const [preview, setPreview] = useState(false);
  const statusClass = invoice.status === "paid" ? "bg-emerald-500/15 text-emerald-300" : invoice.status === "overdue" ? "bg-red-500/15 text-red-300" : invoice.status === "sent" ? "bg-blue-500/15 text-blue-300" : "bg-zinc-500/15 text-zinc-300";
  return <div className="blackCard p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-lg font-black">{invoice.invoiceNumber}</p><p className="text-xs text-zinc-500">{invoice.clientName || invoice.property} • {invoice.property}{invoice.unitNumber ? ` — Unit ${invoice.unitNumber}` : ""}</p></div><div className="text-right"><p className="font-black text-green-400">{money(total)}</p><span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase ${statusClass}`}>{invoice.status}</span></div></div><div className="mt-3 grid grid-cols-3 gap-2 text-center"><BalancePill label="Total" value={money(total)} /><BalancePill label="Paid" value={money(invoice.paidAmount)} /><BalancePill label="Open" value={money(open)} gold /></div><div className="mt-3 flex gap-2"><button className="darkButton flex-1" onClick={onEdit}><Pencil size={16} /> Edit</button><button className="darkButton" onClick={() => setPreview(!preview)}><Eye size={16} /> View</button><button className="iconDanger" onClick={onDelete}><Trash2 size={18} /></button></div>{preview && <div className="printArea mt-4 rounded-2xl border border-white/10 bg-white p-4 text-black"><div className="flex justify-between gap-3"><div><h2 className="text-xl font-black">1 Stop Turnover Specialist LLC</h2><p className="text-sm">Invoice {invoice.invoiceNumber}</p></div><div className="text-right"><p>Status: {invoice.status.toUpperCase()}</p><p>Date: {invoice.invoiceDate}</p><p>Due: {invoice.dueDate}</p></div></div><div className="mt-4"><p className="font-bold">Bill To: {invoice.clientName}</p><p>{invoice.property}{invoice.unitNumber ? ` — Unit ${invoice.unitNumber}` : ""}</p></div><table className="mt-4 w-full text-left text-sm"><thead><tr className="border-b"><th className="py-2">Description</th><th>Qty</th><th>Rate</th><th>Total</th></tr></thead><tbody>{invoice.lineItems.map((item) => <tr key={item.id} className="border-b"><td className="py-2">{item.description}</td><td>{item.qty}</td><td>{money(item.rate)}</td><td>{money(item.qty * item.rate)}</td></tr>)}</tbody></table><div className="mt-4 text-right"><p className="text-lg font-black">Total: {money(total)}</p><p>Paid: {money(invoice.paidAmount)}</p><p>Balance: {money(open)}</p></div>{invoice.notes && <p className="mt-4 text-sm">Notes: {invoice.notes}</p>}<div className="noPrint mt-4 flex gap-2"><button className="goldButton" onClick={() => window.print()}><Printer size={16} /> Print / Save PDF</button><button className="darkButton" onClick={() => onUpdate({ ...invoice, paidAmount: total, status: "paid" })}><Check size={16} /> Mark Paid</button></div></div>}</div>;
}
...
function MoneyInput({ value, onValueChange, placeholder = "0.00" }: { value: number; onValueChange: (value: number) => void; placeholder?: string }) { return <input className="inputElite" type="number" inputMode="decimal" min="0" step="0.01" value={value} placeholder={placeholder} onChange={(e) => onValueChange(safeNumber(e.target.value))} />; }
