...
export default function PayrollProEliteOperations CenterX() {
    ...
    <div className={appShellClass}>
      <div className="mx-auto max-w-[540px] px-4 pb-28 pt-3">
        ...
        {(activeTab === "office" || activeTab === "invoices" || activeTab === "reports") && (
          <>
            <div className="mt-6 flex items-center justify-between">
              <h2 className="text-sm font-black uppercase tracking-wide text-zinc-300">Back Office Pipeline</h2>
            ...
            <div className="mt-3 rounded-2xl border border-green-400/20 bg-green-500/10 p-3 text-xs font-semibold text-green-200">
              Back Office shows the money side: payroll cost, employee balances, invoices, and company charges. Operations Center stays clean for job entry and photos.
            </div>
          </>
        )}
        ...
        <div className="sticky top-[68px] z-20 -mx-4 mt-5 border-y border-white/10 bg-[#02070a]/90 px-4 py-3 shadow-[0_18px_40px_rgba(0,0,0,0.35)] backdrop-blur-xl">
          ...
          <p className="mt-2 text-xs text-zinc-500">{activeTab === "field" ? <><span className="font-black text-green-400">Operations Center mode:</span> jobs, worker pay, units, make ready, and photos.</> : activeTab === "office" || activeTab === "invoices" ? <><span className="font-black text-green-400">Back Office mode:</span> invoices, company charges, customer balances, and billing.</> : <>Historical work orders stay saved.</>}</p>
        </div>
        ...
        <main className="mt-5">
          ...
          {(activeTab === "field" || activeTab === "ops" || activeTab === "jobs" || activeTab === "makeReady") && (
            <section className="space-y-4">
              <SectionTop title="Operations Center" subtitle="Create jobs, track worker pay, upload photos, and manage make ready work.">
                <button
                  onClick={() =>
                    opsView === "makeReady"
                ...
                >
                  <Plus size={18} /> {opsView === "makeReady" ? "Add Unit" : opsView === "assignments" ? "Assign Job" : "New Work Order"}
                </button>
              </SectionTop>

              <div className="rounded-2xl border border-blue-400/20 bg-blue-500/10 p-3 text-sm font-semibold text-blue-200">
                Operations Center is your job-cost side: worker pay, job details, assignments, units, photos, make ready, and one-tap invoice creation.
              </div>
          ...
          {(activeTab === "office" || activeTab === "invoices") && (
            <section className="space-y-4">
              <SectionTop title="Back Office" subtitle="Manage the complete office pipeline: Ready to Invoice, Invoiced, Outstanding, and Paid.">
              ...
              <div className="rounded-2xl border border-green-400/20 bg-green-500/10 p-3 text-sm font-semibold text-green-200">
                Back Office is your billing side: what you charge the company is separate from what you pay the employee.
              </div>
...
function AppMobileHeader({ companyName, activeTab, setActiveTab }: { companyName: string; activeTab: ActiveTab; setActiveTab: (tab: ActiveTab) => void }) {
  ...
  const menuItems: { tab: ActiveTab; label: string; subtitle: string; icon: React.ReactNode }[] = [
    ...
    { tab: "field", label: "Operations Center", subtitle: "Work Orders, pay, photos, make ready", icon: <BriefcaseBusiness size={20} /> },
    { tab: "office", label: "Back Office", subtitle: "Back Office Pipeline, charges, billing", icon: <ReceiptText size={20} /> },
      ...
      <header className="sticky top-0 z-30 -mx-4 border-b border-white/10 bg-[#02070a]/92 px-4 pb-3 pt-3 shadow-[0_12px_35px_rgba(0,0,0,0.38)] backdrop-blur-xl">
        <div className="flex items-center justify-between">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0"><h1 className="truncate text-lg font-black leading-tight">1 Stop Ops Pro</h1><p className="truncate text-xs font-semibold text-zinc-400">Payroll • Make Ready • Back Office Pipeline</p></div>
          </div>
...
function AssignmentModal({ employees, properties, getAddressForProperty, initial, onClose, onSave }: { employees: Employee[]; properties: string[]; getAddressForProperty: (property: string) => string; initial: WorkAssignment | null; onClose: () => void; onSave: (assignment: WorkAssignment) => void }) {
  ...
  return <Modal title={initial ? "Edit Assignment" : "Assign Job"} onClose={onClose}><div className="space-y-3"><div className="grid grid-cols-2 gap-2"><Operations Center label="Employee"><select className="inputElite" value={assignment.employeeId} onChange={(e) => setAssignment({ ...assignment, employeeId: e.target.value })}>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select></Operations Center><Operations Center label="Date"><input className="inputElite" type="date" value={assignment.date} onChange={(e) => setAssignment({ ...assignment, date: e.target.value })} /></Operations Center></div><Operations Center label="Property"><select className="inputElite" value={assignment.property} onChange={(e) => { const selected = e.target.value; setAssignment({ ...assignment, property: selected, address: getAddressForProperty(selected) }); }}>{properties.map((property) => <option key={property} value={property}>{property}</option>)}</select></Operations Center><Operations Center label="Address"><input className="inputElite" value={assignment.address} onChange={(e) => setAssignment({ ...assignment, address: e.target.value })} placeholder="460 Charles St, Providence RI" /></Operations Center><div className="grid grid-cols-2 gap-2"><Operations Center label="Unit"><input className="inputElite" value={assignment.unitNumber} onChange={(e) => setAssignment({ ...assignment, unitNumber: e.target.value })} placeholder="107" /></Operations Center><Operations Center label="Priority"><select className="inputElite" value={assignment.priority} onChange={(e) => setAssignment({ ...assignment, priority: e.target.value as WorkAssignment["priority"] })}><option value="normal">Normal</option><option value="urgent">Urgent</option></select></Operations Center></div><div className="grid grid-cols-2 gap-2"><Operations Center label="Language"><select className="inputElite" value={assignment.language} onChange={(e) => updateLanguage(e.target.value as AssignmentLanguage)}><option value="spanish">Español</option><option value="english">English</option><option value="both">Both</option></select></Operations Center><Operations Center label="Status"><select className="inputElite" value={assignment.status} onChange={(e) => setAssignment({ ...assignment, status: e.target.value as AssignmentStatus })}><option value="assigned">Assigned</option><option value="sent">Sent</option><option value="in-progress">In Progress</option><option value="completed">Completed</option><option value="approved">Approved</option><option value="ready-to-invoice">Ready To Invoice</option></select></Operations Center></div><Operations Center label="Select Job Assignment"><select className="inputElite" value={selectedPreset} onChange={(e) => applyPreset(e.target.value)}><option value="">Choose assignment type...</option>{assignmentPresets.map((preset) => <option key={preset.label} value={preset.label}>{preset.label}</option>)}</select></Operations Center><Operations Center label="Scope of Work"><textarea className="inputElite min-h-32" value={assignment.scope} onChange={(e) => { setSelectedPreset("Custom"); setAssignment({ ...assignment, scope: e.target.value }); }} placeholder="Blank until you choose a job assignment or type your custom scope." /></Operations Center><Operations Center label="Notes"><textarea className="inputElite min-h-20" value={assignment.notes} onChange={(e) => setAssignment({ ...assignment, notes: e.target.value })} placeholder="Optional notes for employee" /></Operations Center><div className="rounded-2xl border border-green-400/20 bg-green-500/10 p-3"><p className="mb-2 text-xs font-black uppercase text-green-400">Message Preview</p><pre className="max-h-56 overflow-y-auto whitespace-pre-wrap rounded-xl bg-black/40 p-3 text-xs font-semibold text-zinc-200">{preview}</pre></div><div className="grid grid-cols-2 gap-2"><button className="goldButton" onClick={() => onSave({ ...assignment, address: assignment.address.trim() || getAddressForProperty(assignment.property), createdAt: assignment.createdAt || new Date().toISOString() })}><Check size={18} /> Save</button><button className="darkButton" onClick={() => copyAssignmentMessage(assignment, employeeName)}><ClipboardList size={18} /> Copy</button><button className="darkButton" onClick={() => openAssignmentText(assignment, employee)}><FileText size={18} /> Send Text</button><button className="darkButton" onClick={() => openAssignmentWhatsApp(assignment, employee)}><Mail size={18} /> WhatsApp</button></div></div></Modal>;
}
...
function MakeReadyCard({ item, employee, employees, onEdit, onDelete, onUpdate, onCreateInvoice }: { item: MakeReadyItem; employee?: Employee; employees: Employee[]; onEdit: () => void; onDelete: () => void; onUpdate: (item: MakeReadyItem) => void; onCreateInvoice: () => void }) {
  ...
  return <div className="blackCard p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-lg font-black">{makeReadyTitle(item)}</p><p className="text-xs text-zinc-500">Assigned: {employee?.name || "Not assigned"} • Deadline: {item.deadline || "—"}</p></div><span className={`rounded-full px-3 py-1 text-[10px] font-black uppercase ${statusClasses}`}>{item.priority === "urgent" ? "Urgent" : item.status.replace("-", " ")}</span></div><div className="mt-3 h-3 overflow-hidden rounded-full border border-white/10 bg-black/40"><div className="h-full rounded-full bg-green-500" style={{ width: `${percent}%` }} /></div><p className="mt-1 text-xs font-bold text-zinc-500">{percent}% complete • Move-out {item.moveOutDate || "—"} • Move-in {item.moveInDate || "—"}</p><div className="mt-3 grid gap-2">{item.tasks.map((task) => <button type="button" key={task.id} onClick={() => { if (!task.done && !confirmAction(`Confirm: mark ${task.label} complete?`)) return; onUpdate({ ...item, tasks: item.tasks.map((row) => row.id === task.id ? { ...row, done: !row.done } : row) }); }} className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-left text-sm font-bold ${task.done ? "border-green-400/25 bg-green-500/10 text-green-300" : "border-zinc-800 bg-black/30 text-zinc-400"}`}><span>{task.done ? "✅" : "⬜"}</span>{task.label}</button>)}</div><div className="mt-3 grid grid-cols-2 gap-2"><Operations Center label="Status"><select className="inputElite" value={item.status} onChange={(e) => { const nextStatus = e.target.value as MakeReadyItem["status"]; if (nextStatus === "ready" && !confirmAction("Confirm: mark this unit ready?")) return; onUpdate({ ...item, status: nextStatus }); }}><option value="scheduled">Scheduled</option><option value="in-progress">In Progress</option><option value="waiting">Waiting</option><option value="ready">Ready</option></select></Operations Center><Operations Center label="Assigned"><select className="inputElite" value={item.assignedEmployeeId} onChange={(e) => onUpdate({ ...item, assignedEmployeeId: e.target.value })}><option value="">Not assigned</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select></Operations Center></div>{item.notes && <p className="mt-3 rounded-2xl border border-white/10 bg-black/25 p-3 text-sm text-zinc-400">{item.notes}</p>}<div className="mt-3 grid grid-cols-2 gap-2"><button className="darkButton" onClick={onEdit}><Pencil size={16} /> Edit Unit</button><button className="goldButton" onClick={onCreateInvoice}><ReceiptText size={16} /> Create Invoice</button></div><div className="mt-2 flex justify-end"><button className="iconDanger" onClick={onDelete}><Trash2 size={18} /></button></div></div>;
}
...
function InvoicesPanel({ invoices, jobs, weekJobs, onAdd, onCreateFromWeek, onEdit, onDelete, onUpdate }: { invoices: Invoice[]; jobs: JobEntry[]; weekJobs: JobEntry[]; onAdd: () => void; onCreateFromWeek: () => void; onEdit: (invoice: Invoice) => void; onDelete: (id: string) => void; onUpdate: (invoice: Invoice) => void }) {
  ...
  const outstandingBack Office Pipeline = invoices.filter((invoice) => !invoiceIsPaid(invoice));
  const paidBack Office Pipeline = invoices.filter((invoice) => invoiceIsPaid(invoice));
  const visibleBack Office Pipeline = invoiceFilter === "paid" ? paidBack Office Pipeline : invoiceFilter === "outstanding" ? outstandingBack Office Pipeline : invoices;
  const openBalance = invoices.reduce((sum, invoice) => sum + Math.max(invoiceTotal(invoice) - safeNumber(invoice.paidAmount), 0), 0);
  const paidCount = paidBack Office Pipeline.length;

  const filterButtons = [
    { id: "all" as const, label: "All Back Office Pipeline", count: invoices.length },
    { id: "outstanding" as const, label: "Outstanding", count: outstandingBack Office Pipeline.length },
    { id: "paid" as const, label: "Paid", count: paidBack Office Pipeline.length },
  ];

  return (
    <section className="space-y-4">
      <SectionTop title="Back Office Pipeline" subtitle="View all invoices, outstanding balances, paid invoices, preview PDF, print, email, and track payments.">
      ...
      <div className="grid grid-cols-3 gap-2">
        <MiniMetric label="Back Office Pipeline" value={invoices.length} />
      ...
      <div className="space-y-3">
        {visibleBack Office Pipeline.map((invoice) => <InvoiceCard key={invoice.id} invoice={invoice} jobs={jobs} onEdit={() => onEdit(invoice)} onDelete={() => onDelete(invoice.id)} onUpdate={onUpdate} />)}
        {visibleBack Office Pipeline.length === 0 && <div className="blackCard p-6"><EmptyText text={invoiceFilter === "paid" ? "No paid invoices yet." : invoiceFilter === "outstanding" ? "No outstanding invoices right now." : "No invoices yet. Create one manually or from this week’s jobs."} /></div>}
      </div>
    </section>
  );
}
...
function EmployeeModal({ onClose, onSave }: { onClose: () => void; onSave: (employee: Employee) => void }) {
  ...
  return <Modal title="Add Employee" onClose={onClose}><div className="space-y-3"><Operations Center label="Employee Name"><input className="inputElite" value={employee.name} onChange={(e) => setEmployee({ ...employee, name: e.target.value })} placeholder="Worker name" /></Operations Center><Operations Center label="Phone"><input className="inputElite" value={employee.phone} onChange={(e) => setEmployee({ ...employee, phone: e.target.value })} placeholder="Phone number" /></Operations Center><Operations Center label="Default Rate"><MoneyInput value={employee.defaultRate} onValueChange={(value) => setEmployee({ ...employee, defaultRate: value })} /></Operations Center><Operations Center label="Notes"><textarea className="inputElite min-h-20" value={employee.notes} onChange={(e) => setEmployee({ ...employee, notes: e.target.value })} /></Operations Center><button className="goldButton w-full" onClick={() => employee.name.trim() && onSave({ ...employee, name: employee.name.trim() })}><Check size={18} /> Save Employee</button></div></Modal>;
}
...
function JobModal({ employees, properties, jobTypeOptions, getAddressForProperty, onAddProperty, onClose, onSave }: { employees: Employee[]; properties: string[]; jobTypeOptions: string[]; getAddressForProperty: (property: string) => string; onAddProperty: (property: string) => void; onClose: () => void; onSave: (job: JobEntry, assignment?: WorkAssignment) => void }) {
  ...
  return <Modal title="New Work Order Entry" onClose={onClose}><div className="space-y-3"><div className="rounded-2xl border border-green-400/20 bg-green-500/10 p-3 text-sm font-semibold text-green-100"><b>One entry workflow:</b> this saves the payroll job and can create the employee assignment message at the same time.</div><Operations Center label="Employee / Assigned To"><select className="inputElite" value={employeeId} onChange={(e) => setEmployeeId(e.target.value)}>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select></Operations Center><div className="grid grid-cols-2 gap-3"><Operations Center label="Date"><input className="inputElite cursor-pointer" type="date" value={date} onClick={(e) => e.currentTarget.showPicker?.()} onFocus={(e) => e.currentTarget.showPicker?.()} onChange={(e) => setDate(e.target.value)} /></Operations Center><Operations Center label="Property"><select className="inputElite" value={property} onChange={(e) => { const selected = e.target.value; if (selected === "__add_new_property__") { const entered = window.prompt("Enter new property name:"); const cleanProperty = entered?.trim() || ""; if (cleanProperty) { onAddProperty(cleanProperty); setProperty(cleanProperty); setAssignmentAddress(""); } return; } setProperty(selected); setAssignmentAddress(getAddressForProperty(selected)); }}>{properties.map((item) => <option key={item} value={item}>{item}</option>)}<option value="__add_new_property__">+ Add New Property</option></select></Operations Center></div><Operations Center label="Unit #"><input className="inputElite" value={unitNumber} onChange={(e) => setUnitNumber(e.target.value)} placeholder="Example: 212" /></Operations Center><Operations Center label="Work Type"><div className="grid grid-cols-2 gap-2">{jobTypeOptions.map((type) => <button type="button" key={type} onClick={() => toggleType(type)} className={`rounded-xl border px-3 py-2 text-left text-xs font-bold ${selectedTypes.includes(type) ? "border-green-400/50 bg-green-500/20 text-green-300" : "border-zinc-800 bg-black/30 text-zinc-400"}`}>{selectedTypes.includes(type) ? "✓ " : ""}{type}</button>)}</div></Operations Center><Operations Center label="Custom Work"><input className="inputElite" value={customWork} onChange={(e) => setCustomWork(e.target.value)} placeholder="Extra work description" /></Operations Center><div className="grid grid-cols-2 gap-3"><Operations Center label="Pay"><MoneyInput value={pay} onValueChange={setPay} placeholder="Enter Amount" /></Operations Center><Operations Center label="Paid"><MoneyInput value={paidAmount} onValueChange={setPaidAmount} placeholder="Enter Amount" /></Operations Center></div><div className="rounded-2xl border border-white/10 bg-black/25 p-3"><label className="flex items-center gap-3 text-sm font-black text-zinc-100"><input type="checkbox" checked={createAssignment} onChange={(e) => setCreateAssignment(e.target.checked)} /> Create employee assignment message</label>{createAssignment && <div className="mt-3 space-y-3"><Operations Center label="Job Address for Message"><input className="inputElite" value={assignmentAddress} onChange={(e) => setAssignmentAddress(e.target.value)} placeholder="460 Charles St, Providence RI" /></Operations Center><div className="grid grid-cols-2 gap-3"><Operations Center label="Priority"><select className="inputElite" value={assignmentPriority} onChange={(e) => setAssignmentPriority(e.target.value as WorkAssignment["priority"])}><option value="normal">Normal</option><option value="urgent">Urgent</option></select></Operations Center><Operations Center label="Language"><select className="inputElite" value={assignmentLanguage} onChange={(e) => setAssignmentLanguage(e.target.value as AssignmentLanguage)}><option value="spanish">Español</option><option value="english">English</option><option value="both">Both</option></select></Operations Center></div><div className="rounded-2xl border border-green-400/20 bg-green-500/10 p-3"><p className="mb-2 text-xs font-black uppercase text-green-400">Message Preview</p><pre className="max-h-48 overflow-y-auto whitespace-pre-wrap rounded-xl bg-black/40 p-3 text-xs font-semibold text-zinc-200">{buildAssignmentMessage(assignmentPreview, assignedEmployee?.name || "")}</pre></div></div>}</div><Operations Center label="Photos"><div className="rounded-2xl border border-dashed border-zinc-800 bg-black/30 p-4"><div className="grid grid-cols-2 gap-2"><label className="goldButton w-full cursor-pointer"><Camera size={18} /> Take Photo<input type="file" accept="image/*" multiple capture="environment" className="hidden" onChange={async (e) => { const newPhotos = await readPhotoFiles(e.target.files); setPhotos((prev) => [...prev, ...newPhotos]); e.currentTarget.value = ""; }} /></label><label className="darkButton w-full cursor-pointer"><ImageIcon size={18} /> Upload<input type="file" accept="image/*" multiple className="hidden" onChange={async (e) => { const newPhotos = await readPhotoFiles(e.target.files); setPhotos((prev) => [...prev, ...newPhotos]); e.currentTarget.value = ""; }} /></label></div>{photos.length > 0 && <p className="mt-3 text-center text-sm text-zinc-400">{photos.length} photo(s) attached.</p>}</div></Operations Center><Operations Center label="Notes"><textarea className="inputElite min-h-24" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Notes" /></Operations Center><button className="goldButton w-full" onClick={saveJobAndAssignment}><Check size={18} /> Save Job{createAssignment ? " + Assignment" : ""}</button></div></Modal>;
}
...
function MakeReadyModal({ employees, properties, initial, onClose, onSave }: { employees: Employee[]; properties: string[]; initial: MakeReadyItem | null; onClose: () => void; onSave: (item: MakeReadyItem) => void }) {
  ...
  return <Modal title={initial ? "Edit Make Ready Unit" : "Add Make Ready Unit"} onClose={onClose}><div className="space-y-3"><div className="grid grid-cols-2 gap-3"><Operations Center label="Property"><select className="inputElite" value={item.property} onChange={(e) => setItem({ ...item, property: e.target.value })}>{properties.map((property) => <option key={property} value={property}>{property}</option>)}</select></Operations Center><Operations Center label="Unit #"><input className="inputElite" value={item.unitNumber} onChange={(e) => setItem({ ...item, unitNumber: e.target.value })} /></Operations Center><Operations Center label="Assigned"><select className="inputElite" value={item.assignedEmployeeId} onChange={(e) => setItem({ ...item, assignedEmployeeId: e.target.value })}><option value="">Not assigned</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select></Operations Center><Operations Center label="Priority"><select className="inputElite" value={item.priority} onChange={(e) => setItem({ ...item, priority: e.target.value as MakeReadyItem["priority"] })}><option value="normal">Normal</option><option value="urgent">Urgent</option></select></Operations Center><Operations Center label="Move-Out"><input className="inputElite" type="date" value={item.moveOutDate} onChange={(e) => setItem({ ...item, moveOutDate: e.target.value })} /></Operations Center><Operations Center label="Move-In"><input className="inputElite" type="date" value={item.moveInDate} onChange={(e) => setItem({ ...item, moveInDate: e.target.value })} /></Operations Center><Operations Center label="Deadline"><input className="inputElite" type="date" value={item.deadline} onChange={(e) => setItem({ ...item, deadline: e.target.value })} /></Operations Center><Operations Center label="Status"><select className="inputElite" value={item.status} onChange={(e) => setItem({ ...item, status: e.target.value as MakeReadyItem["status"] })}><option value="scheduled">Scheduled</option><option value="in-progress">In Progress</option><option value="waiting">Waiting</option><option value="ready">Ready</option></select></Operations Center></div><Operations Center label="Checklist"><div className="space-y-2">{item.tasks.map((task) => <div key={task.id} className="grid grid-cols-[auto_1fr_auto] items-center gap-2 rounded-xl border border-zinc-800 bg-black/25 p-2"><input type="checkbox" checked={task.done} onChange={() => setItem({ ...item, tasks: item.tasks.map((row) => row.id === task.id ? { ...row, done: !row.done } : row) })} /><input className="inputElite !py-2" value={task.label} onChange={(e) => setItem({ ...item, tasks: item.tasks.map((row) => row.id === task.id ? { ...row, label: e.target.value } : row) })} /><button className="iconDanger !p-2" onClick={() => setItem({ ...item, tasks: item.tasks.filter((row) => row.id !== task.id) })}><Trash2 size={14} /></button></div>)}<button className="darkButton w-full" onClick={() => setItem({ ...item, tasks: [...item.tasks, { id: uid(), label: "New Task", done: false }] })}><Plus size={16} /> Add Task</button></div></Operations Center><Operations Center label="Notes"><textarea className="inputElite min-h-24" value={item.notes} onChange={(e) => setItem({ ...item, notes: e.target.value })} /></Operations Center><button className="goldButton w-full" onClick={() => onSave(item)}><Check size={18} /> Save Make Ready</button></div></Modal>;
}
...
function InvoiceModal({ invoices, properties, getProfileForProperty, initial, onClose, onSave }: { invoices: Invoice[]; properties: string[]; getProfileForProperty: (property: string) => PropertyContactProfile; initial: Invoice | null; onClose: () => void; onSave: (invoice: Invoice) => void }) {
  ...
  const total = invoiceTotal(invoice);
  return <Modal title={initial ? "Edit Invoice" : "New Invoice"} onClose={onClose}><div className="space-y-3"><div className="grid grid-cols-2 gap-3"><Operations Center label="Invoice #"><input className="inputElite" value={invoice.invoiceNumber} onChange={(e) => setInvoice({ ...invoice, invoiceNumber: e.target.value })} /></Operations Center><Operations Center label="Status"><select className="inputElite" value={invoice.status} onChange={(e) => setInvoice({ ...invoice, status: e.target.value as Invoice["status"] })}><option value="due">Due</option><option value="sent">Sent</option><option value="paid">Paid</option><option value="overdue">Overdue</option></select></Operations Center><Operations Center label="Client"><input className="inputElite" value={invoice.clientName} onChange={(e) => setInvoice({ ...invoice, clientName: e.target.value })} placeholder="Example: Wingate Companies" /></Operations Center><Operations Center label="Client Email"><input className="inputElite" type="email" value={invoice.clientEmail || ""} onChange={(e) => setInvoice({ ...invoice, clientEmail: e.target.value })} placeholder="manager@email.com" /></Operations Center><Operations Center label="Property"><select className="inputElite" value={invoice.property} onChange={(e) => { const selected = e.target.value; const profile = getProfileForProperty(selected); setInvoice({ ...invoice, property: selected, propertyAddress: profile.address || "", clientName: invoice.clientName || profile.billingName || selected, clientEmail: invoice.clientEmail || profile.email || "" }); }}>{properties.map((property) => <option key={property} value={property}>{property}</option>)}</select></Operations Center><Operations Center label="Unit #"><input className="inputElite" value={invoice.unitNumber} onChange={(e) => setInvoice({ ...invoice, unitNumber: e.target.value })} /></Operations Center><Operations Center label="Invoice Date"><input className="inputElite" type="date" value={invoice.invoiceDate} onChange={(e) => setInvoice({ ...invoice, invoiceDate: e.target.value })} /></Operations Center><Operations Center label="Due Date"><input className="inputElite" type="date" value={invoice.dueDate} onChange={(e) => setInvoice({ ...invoice, dueDate: e.target.value })} /></Operations Center><Operations Center label="Paid Amount"><MoneyInput value={invoice.paidAmount} onValueChange={(value) => { const cleanValue = safeNumber(value); if (cleanValue >= total && total > 0) { if (!confirmAction("Confirm: mark this invoice paid?")) return; setInvoice({ ...invoice, paidAmount: cleanValue, status: "paid" }); return; } setInvoice({ ...invoice, paidAmount: cleanValue, status: invoice.status === "paid" ? "sent" : invoice.status }); }} /></Operations Center></div><Operations Center label="Line Items"><div className="space-y-3">{invoice.lineItems.map((line) => <div key={line.id} className="rounded-2xl border border-zinc-800 bg-black/25 p-3"><Operations Center label="Description"><input className="inputElite" value={line.description} onChange={(e) => setInvoice({ ...invoice, lineItems: invoice.lineItems.map((item) => item.id === line.id ? { ...item, description: e.target.value } : item) })} /></Operations Center><div className="mt-2 grid grid-cols-[1fr_1fr_auto] gap-2"><Operations Center label="Qty"><input className="inputElite" type="number" value={line.qty} onChange={(e) => setInvoice({ ...invoice, lineItems: invoice.lineItems.map((item) => item.id === line.id ? { ...item, qty: safeNumber(e.target.value) } : item) })} /></Operations Center><Operations Center label="Rate"><MoneyInput value={line.rate} onValueChange={(value) => setInvoice({ ...invoice, lineItems: invoice.lineItems.map((item) => item.id === line.id ? { ...item, rate: value } : item) })} /></Operations Center><button className="iconDanger self-end" onClick={() => { if (!confirmAction("Remove this line item from the invoice?")) return; setInvoice({ ...invoice, lineItems: invoice.lineItems.filter((item) => item.id !== line.id) }); }}><Trash2 size={16} /></button></div></div>)}<button className="darkButton w-full" onClick={() => setInvoice({ ...invoice, lineItems: [...invoice.lineItems, { id: uid(), description: "New line item", qty: 1, rate: 0 }] })}><Plus size={16} /> Add Line Item</button></div></Operations Center><div className="rounded-2xl border border-green-400/20 bg-green-500/10 p-3 text-right"><p className="text-xs font-black uppercase text-green-400">Invoice Total</p><p className="text-2xl font-black">{money(total)}</p><p className="mt-1 text-xs font-semibold text-zinc-400">Phase 4 designer will use this amount on the PDF-ready preview.</p></div><div className="rounded-2xl border border-blue-400/20 bg-blue-500/10 p-3 text-xs font-semibold text-blue-100"><b>Preview PDF Tip:</b> Add before/after photos here, then tap Preview PDF on the invoice card to review the professional layout before printing, saving as PDF, or emailing.</div><Operations Center label="Before Photos"><InvoicePhotoPicker photos={invoice.beforePhotos || []} label="Add Before Photos" onChange={(photos) => setInvoice({ ...invoice, beforePhotos: photos })} /></Operations Center><Operations Center label="After / Completed Job Photos"><InvoicePhotoPicker photos={invoice.afterPhotos || []} label="Add After Photos" onChange={(photos) => setInvoice({ ...invoice, afterPhotos: photos })} /></Operations Center><Operations Center label="Notes / Terms"><textarea className="inputElite min-h-24" value={invoice.notes} onChange={(e) => setInvoice({ ...invoice, notes: e.target.value })} /></Operations Center><div className="grid grid-cols-2 gap-2"><button className="goldButton" onClick={() => { const cleanTotal = invoiceTotal(invoice); const cleanPaid = safeNumber(invoice.paidAmount); const cleanStatus = invoice.status === "paid" && !(cleanTotal > 0 && cleanPaid >= cleanTotal) ? "sent" : invoice.status; onSave({ ...invoice, paidAmount: cleanPaid, status: cleanStatus }); }}><Check size={18} /> Save Invoice</button><button className="darkButton" onClick={() => { const cleanTotal = invoiceTotal(invoice); const cleanPaid = safeNumber(invoice.paidAmount); const cleanInvoice = { ...invoice, status: invoice.status === "paid" && !(cleanTotal > 0 && cleanPaid >= cleanTotal) ? "sent" as Invoice["status"] : invoice.status, paidAmount: cleanPaid }; setInvoice(cleanInvoice); printInvoiceDocument(cleanInvoice, cleanInvoice.beforePhotos || [], cleanInvoice.afterPhotos || []); }}><Eye size={18} /> Preview PDF</button><button className="darkButton col-span-2" onClick={() => openInvoiceEmail({ ...invoice, status: invoice.status === "paid" && !(invoiceTotal(invoice) > 0 && safeNumber(invoice.paidAmount) >= invoiceTotal(invoice)) ? "sent" : invoice.status })}><Mail size={18} /> Email After Preview</button></div></div></Modal>;
}
...
function PropertyModal({ initialName = "", initialProfile, onClose, onSave }: { initialName?: string; initialProfile?: PropertyContactProfile; onClose: () => void; onSave: (property: string, profile: PropertyContactProfile) => void }) {
  ...
  return <Modal title={initialName ? "Edit Property Profile" : "Add Property Profile"} onClose={onClose}><div className="space-y-3"><Operations Center label="Property Name"><input className="inputElite" value={property} onChange={(e) => { setProperty(e.target.value); if (!profile.billingName) setProfile({ ...profile, billingName: e.target.value }); }} placeholder="Property name" /></Operations Center><Operations Center label="Full Address"><input className="inputElite" value={profile.address} onChange={(e) => setProfile({ ...profile, address: e.target.value })} placeholder="460 Charles St, Providence RI" /></Operations Center><div className="grid grid-cols-2 gap-3"><Operations Center label="Contact Name"><input className="inputElite" value={profile.contactName} onChange={(e) => setProfile({ ...profile, contactName: e.target.value })} placeholder="Manager name" /></Operations Center><Operations Center label="Phone"><input className="inputElite" value={profile.phone} onChange={(e) => setProfile({ ...profile, phone: e.target.value })} placeholder="Phone number" /></Operations Center></div><Operations Center label="Email"><input className="inputElite" type="email" value={profile.email} onChange={(e) => setProfile({ ...profile, email: e.target.value })} placeholder="manager@email.com" /></Operations Center><Operations Center label="Billing Name"><input className="inputElite" value={profile.billingName} onChange={(e) => setProfile({ ...profile, billingName: e.target.value })} placeholder="Company/client name for invoices" /></Operations Center><Operations Center label="Notes"><textarea className="inputElite min-h-20" value={profile.notes} onChange={(e) => setProfile({ ...profile, notes: e.target.value })} placeholder="Gate codes, office notes, billing instructions, etc." /></Operations Center><button className="goldButton w-full" onClick={() => { const cleanName = property.trim(); if (!cleanName) return; onSave(cleanName, { address: profile.address.trim(), contactName: profile.contactName.trim(), email: profile.email.trim(), phone: profile.phone.trim(), billingName: profile.billingName.trim() || cleanName, notes: profile.notes.trim() }); }}><Check size={18} /> Save Property Profile</button></div></Modal>; }
...
function Operations Center({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="labelElite">{label}</span>{children}</label>; }
...
function BottomNav({ activeTab, setActiveTab }: { activeTab: ActiveTab; setActiveTab: (tab: ActiveTab) => void }) {
  const tabs: { tab: ActiveTab; label: string; icon: React.ReactNode }[] = [
    ...
    { tab: "field", label: "Operations Center", icon: <BriefcaseBusiness size={20} /> },
    { tab: "office", label: "Back Office", icon: <ReceiptText size={20} /> },
