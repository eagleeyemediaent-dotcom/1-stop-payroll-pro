...
// PHASE 15 single-file replacement for app/page.tsx
// Work Order conversion built on the real working Phase 13 foundation
// Property address persistence + assignment preset dropdown fix retained
      ...
      <div className="mx-auto max-w-[540px] px-4 pb-28 pt-3">
        ...
        {activeTab === "dashboard" && (
          <>
            ...
            <p className="mt-3 text-center text-xs font-semibold text-zinc-500">Work orders, payroll, turnovers, make ready, and invoices in one app.</p>
          </>
        )}
        ...
        {(activeTab === "office" || activeTab === "invoices" || activeTab === "reports") && (
          <>
            ...
            <div className="mt-3 rounded-2xl border border-green-400/20 bg-green-500/10 p-3 text-xs font-semibold text-green-200">
              Office shows the money side: payroll cost, employee balances, invoices, and company charges. Field stays clean for work orders and photos.
            </div>
          </>
        )}
        ...
        <div className="sticky top-[68px] z-20 -mx-4 mt-5 border-y border-white/10 bg-[#02070a]/90 px-4 py-3 shadow-[0_18px_40px_rgba(0,0,0,0.35)] backdrop-blur-xl">
          ...
          <p className="mt-2 text-xs text-zinc-500">{activeTab === "field" ? <><span className="font-black text-green-400">Field mode:</span> work orders, worker pay, units, make ready, and photos.</> : activeTab === "office" || activeTab === "invoices" ? <><span className="font-black text-green-400">Office mode:</span> invoices, company charges, customer balances, and billing.</> : <>Historical work orders stay saved.</>}</p>
        </div>
        ...
        <main className="mt-5">
          ...
          {(activeTab === "field" || activeTab === "ops" || activeTab === "jobs" || activeTab === "makeReady") && (
            <section className="space-y-4">
              <SectionTop title="Field" subtitle="Create work orders, track worker pay, upload photos, and manage make ready work.">
                <button
                  onClick={() =>
                    opsView === "makeReady"
              ...
              <div className="rounded-2xl border border-blue-400/20 bg-blue-500/10 p-3 text-sm font-semibold text-blue-200">
                Field is your work-order side: worker pay, assignment details, units, photos, make ready, and one-tap invoice creation.
              </div>
...
function AppMobileHeader({ companyName, activeTab, setActiveTab }: { companyName: string; activeTab: ActiveTab; setActiveTab: (tab: ActiveTab) => void }) {
  ...
  const menuItems: { tab: ActiveTab; label: string; subtitle: string; icon: React.ReactNode }[] = [
    ...
    { tab: "field", label: "Field", subtitle: "Work Orders, pay, photos, make ready", icon: <BriefcaseBusiness size={20} /> },
...
Mark this work order as UNPAID?
...
function InvoicesPanel({ invoices, jobs, weekWork Orders, onAdd, onCreateFromWeek, onEdit, onDelete, onUpdate }: { invoices: Invoice[]; jobs: JobEntry[]; weekWork Orders: JobEntry[]; onAdd: () => void; onCreateFromWeek: () => void; onEdit: (invoice: Invoice) => void; onDelete: (id: string) => void; onUpdate: (invoice: Invoice) => void }) {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        {visibleInvoices.length === 0 && <div className="blackCard p-6"><EmptyText text={invoiceFilter === "paid" ? "No paid invoices yet." : invoiceFilter === "outstanding" ? "No outstanding invoices right now." : "No invoices yet. Create one manually or from this week’s work orders."} /></div>}
      </div>
    </section>
  );
}
    ...
    <div className="printArea mt-4 overflow-hidden rounded-2xl border border-white/10 bg-white text-black shadow-2xl">
      ...
      <div className="p-4">
        ...
        {(beforePhotos.length > 0 || afterPhotos.length > 0) && (
          <div className="mt-4">
            <h3 className="font-black">Work Order Photos</h3>
