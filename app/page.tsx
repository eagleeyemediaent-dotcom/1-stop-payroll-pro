...
function openInvoiceMessage(invoice: Invoice) {
  invoice = customerSafeInvoice(invoice);
  alert("The invoice PDF will open first. Save it as a PDF, then attach that clean PDF to your text link.");
  printInvoiceDocument(invoice, invoice.beforePhotos || [], invoice.afterPhotos || []);
  const text = encodeURIComponent(`View Invoice PDF: Invoice ${invoice.invoiceNumber} is ready from 1 Stop Turnover Specialist LLC. Thank you.`);
  ...
  <main class="invoice">
    ...
    <section class="meta">
      <div>
        ...
        <p class="strong">${escapePrintHtml(invoice.property || invoice.clientName || "Property Name")}</p>
...
export default function PayrollProEliteOperationsX() {
  ...
  function createInvoiceFromWeekJobs() {
    ...
    const includeJobPhotos = confirmAction("Include work order photos on this invoice?");
  ...
  function createInvoiceFromJob(job: JobEntry) {
    const includeJobPhotos = confirmAction("Include this work order photos on the invoice?");
  ...
  function createEstimateFromJob(job: JobEntry) {
    const includeJobPhotos = confirmAction("Include this work order photos on the estimate?");
...
function JobList({ jobs, employees, employeesById, properties, jobTypeOptions, onDelete, onUpdate, onCreateInvoice, onCreateEstimate, onDuplicate }: { jobs: JobEntry[]; employees: Employee[]; employeesById: Map<string, Employee>; properties: string[]; jobTypeOptions: string[]; onDelete: (id: string) => void; onUpdate: (job: JobEntry) => void; onCreateInvoice: (job: JobEntry) => void; onCreateEstimate: (job: JobEntry) => void; onDuplicate: (job: JobEntry) => void }) {
...
function JobRow({ job, employees, employee, properties, jobTypeOptions, onDelete, onUpdate, onCreateInvoice, onCreateEstimate, onDuplicate }: { job: JobEntry; employees: Employee[]; employee?: Employee; properties: string[]; jobTypeOptions: string[]; onDelete: () => void; onUpdate: (job: JobEntry) => void; onCreateInvoice: () => void; onCreateEstimate: () => void; onDuplicate: () => void }) {
